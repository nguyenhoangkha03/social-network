...
async function sendMessage(friendId, text, imageFile) {
    ...
    if (imageFile) {formData.append('hinhanh', imageFile); console.log(`1111`)}; // <-- key chính xác là "hinhanh"
...
function renderFriendList() {
    ...
    friends.forEach(f => {
        ...
        li.className = 'friend-item' + (selectedFriend && f.id_user === selectedFriend.id_user ? ' selected' : '');
        li.innerHTML = `
            <img src="${f.avatar || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(f.hoten)}" class="friend-avatar" alt="avatar">
            <span>${f.hoten}</span>
        `;
...
function renderMessages(messages) {
    ...
    messages.forEach(msg => {
        ...
        row.className = 'chat-message-row' + (msg.me ? ' chat-message me' : ' chat-message');
        const bubble = document.createElement('div');
        bubble.className = 'chat-message-bubble';

        const isImage = msg.noidung && msg.noidung.startsWith('uploads/messages/');
        if (isImage) {
