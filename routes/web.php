...
Route::get('/chat', function() { return view('friends.chat'); })->name('chat');

// API lấy danh sách bạn bè (tất cả bạn bè)
Route::get('/api/friends', function() {
    if (!session()->has('user_id')) return response()->json(['error'=>'Chưa đăng nhập'], 401);
...
Route::post('/api/chat/send', function(\Illuminate\Http\Request $request) {
     $request->validate([
    ...
    //     'thoigiantao' => now(),
    // ]);
    // return response()->json(['success'=>true, 'message'=>'Đã gửi tin nhắn!', 'msg'=>$msg]);
});
