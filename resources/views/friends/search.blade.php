...
<head>
    ...
    <style>
        ...
        .search-input { flex: 1; padding: 10px 14px; border-radius: 8px; border: 1px solid #ccc; font-size: 1rem; }
        .search-btn { padding: 10px 28px; border-radius: 22px; border: none; background: #007bff; color: #fff; font-weight: 600; font-size: 1rem; cursor: pointer; }
        .user-list { list-style: none; padding: 0; margin: 0; }
        .user-item { display: flex; align-items: center; gap: 16px; padding: 16px 0; border-bottom: 1px solid #f3f3f3; }
        .avatar { width: 48px; height: 48px; border-radius: 50%; background: #f0f0f0; overflow: hidden; display: flex; align-items: center; justify-content: center; }
        .avatar img { width: 100%; height: 100%; object-fit: cover; }
        .avatar i { font-size: 2rem; color: #ccc; }
        .user-info { flex: 1; }
        .user-name { font-weight: 600; font-size: 1.1rem; }
        .user-meta { color: #888; font-size: 0.95rem; }
        .action-btn { padding: 8px 20px; border-radius: 18px; border: none; background: #007bff; color: #fff; font-weight: 500; cursor: pointer; transition: background 0.2s; }
        .action-btn[disabled] { background: #ccc; cursor: not-allowed; }
        .action-btn.message { background: #28a745; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <ul class="user-list">
            ...
            <li class="user-item">
                ...
                @if($currentUser && $u->id_user != $currentUser->id_user)
                    @php
                    ...
                    @endphp
                    @if($isFriend)
                        <a href="{{ route('friends.chat', $u->id_user) }}" class="action-btn message"><i class="fas fa-comments"></i> Nhắn tin</a>
                    @else
                        <button class="action-btn add-friend-btn" data-id="{{ $u->id_user }}"><i class="fas fa-user-plus"></i> Follow</button>
                    @endif
                @endif
            </li>
            @endforeach
